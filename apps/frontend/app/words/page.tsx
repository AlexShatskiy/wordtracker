...
function StatTile({ label, value, tint, color }: { label: string; value: number; tint: string; color: string }) {
  return (
...
export default function WordsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      <AppHeader title="My Words" subtitle="Sorted by lookup count" onPairSwitcherOpen={() => setPairSheetOpen(true)} />
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: '4px 14px 14px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
          ...
          <StatTile label="Stuck" value={stuckCount} tint="color-mix(in oklch, var(--danger) 12%, transparent)" color="var(--danger)" />
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, overflowX: 'auto', scrollbarWidth: 'none', alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={() => setSort(s => s === 'count' ? 'az' : 'count')} style={{
            padding: '6px 12px', borderRadius: 100, fontSize: 11.5, fontWeight: 500,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {list.length === 0 ? (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '28px 14px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--text-hint)', marginTop: 4 }}>Translate one to add it here.</div>
            </div>
          ) : list.map(w => (
            <button key={w.id} onClick={() => setDetailWord(w)} style={{
      ...
      {quickAddOpen && pair && (
        <QuickAddSheet
          ...
          onSave={() => { showToast('Saved to My Words'); loadWords() }}
        />
      )}
      {detailWord && (
        <WordDetailSheet
